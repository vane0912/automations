...
def India_Business_eVisa(data):
    ...
    try:
        for order in range(int(Global_Variables['N. Orders'])):
            ...
            Select(wait.until(EC.visibility_of_element_located((By.XPATH, '//select[@data-handle="dropdown-general.visa_type_id"]')))).select_by_value('22')
            ...
            try:
                ...
                wait.until(lambda driver: driver.current_url != current_url) 
            try:
                questions_loop(10120, browser, wait, order, 1)
                if order == 0:
